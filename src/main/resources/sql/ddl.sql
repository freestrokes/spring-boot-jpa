
DROP TABLE IF EXISTS board;
DROP TABLE IF EXISTS board_comment;

CREATE TABLE board (
    board_id bigint(20) NOT NULL AUTO_INCREMENT COMMENT '게시판 id',
    title varchar(500) DEFAULT NULL COMMENT '제목',
    content text DEFAULT NULL COMMENT '내용',
    author varchar(100) DEFAULT NULL COMMENT '작성자',
    PRIMARY KEY (board_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='게시판';

CREATE TABLE board_comment (
    board_comment_id bigint(20) NOT NULL AUTO_INCREMENT COMMENT '게시판 댓글 id',
    board_id bigint(20) NOT NULL COMMENT '게시판 id',
    content text DEFAULT NULL COMMENT '내용',
    author varchar(100) DEFAULT NULL COMMENT '작성자',
    PRIMARY KEY (board_comment_id),
    CONSTRAINT board_comment_fk1 FOREIGN KEY (board_id) REFERENCES board (board_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='게시판 댓글';
